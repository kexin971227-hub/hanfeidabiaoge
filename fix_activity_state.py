...
def daily_reset_loop():
    while True:
        ...
        print(f"⏰ 距重置还有 {wait_seconds/3600:.1f} 小时")
        time.sleep(wait_seconds)
        conn = get_db()
        cur = conn.cursor()
        cur.execute('UPDATE user_state SET state = NULL, activity = NULL, act_start = NULL')
        conn.commit()
        cur.close()
        conn.close()
        print(f"✅ 每日状态重置")
...
init_db()
print("✅ 机器人运行（自维护 | 活动互斥 | 下班后可再上班 | 凌晨3点重置）")
